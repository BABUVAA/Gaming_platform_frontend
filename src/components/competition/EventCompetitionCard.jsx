...
const EventCompetitionCard = ({ busy, event, onRegister, staffReadOnly }) => {
  ...
  return (
    <article className="rounded-2xl border border-slate-700 bg-slate-900/70 p-5 shadow-[0_14px_34px_rgba(2,8,23,0.18)]">
      ...
      {timeline.target ? (
        <div className="mt-4 rounded-xl border border-cyan-300/15 bg-cyan-300/5 px-4 py-3">
          <p className="text-xs text-slate-400">{timeline.label}</p>
          <p className="mt-1 font-mono text-2xl font-black tracking-tight text-cyan-100">
      ...
      {rewards.length ? (
        <p className="mt-3 text-xs font-bold text-emerald-200">
          ...
          {rewards
            .slice(0, 3)
            .map((reward) => `#${reward.place} ${money(reward.amountMinor)}`)
            .join(" / ")}
          {rewards.length > 3 ? ` / +${rewards.length - 3} places` : ""}
        </p>
      ) : null}
      {mine ? (
        <p className="mt-3 text-sm font-bold capitalize text-cyan-200">
          Your entry: {mine.status.replaceAll("_", " ")}
        </p>
      ) : null}
      {event.execution?.progression?.status &&
      event.execution.progression.status !== "active" ? (
        <p className="mt-2 text-sm font-black capitalize text-amber-200">
          Result: {event.execution.progression.status.replaceAll("_", " ")}
        </p>
      ) : null}
      {event.execution?.myBatch?.matchId ? (
        <Link
          className="mt-4 block rounded-xl border border-cyan-300/30 px-4 py-2.5 text-center text-sm font-black text-cyan-100"
      ...
      <Link
        className="mt-4 block rounded-xl border border-slate-700 px-4 py-2.5 text-center text-sm font-black text-slate-200 hover:border-cyan-300/40 hover:text-cyan-100"
      ...
      {!staffReadOnly && !committed && event.registration?.isOpen ? (
        <button
          className="mt-3 w-full rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50"
...
const Fact = ({ label, value }) => (
  <div className="min-w-0 rounded-xl border border-slate-800 p-3">
    ...
    <p className="mt-1 truncate font-black capitalize text-white">{value}</p>
  </div>
);
Fact.propTypes = {
