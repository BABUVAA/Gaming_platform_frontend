  ...
  return (
    <section className="rounded-3xl border border-cyan-300/20 bg-[#07111f] p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Fact label="Current list" value={currentPlayers.toLocaleString("en-IN")} />
        <Fact label="Rounds created" value={String(stages.length)} />
        <Fact label="Promoted" value={String(latest?.promotedCount || 0)} />
        <Fact label="Eliminated" value={String(latest?.eliminatedCount || 0)} />
      </div>
          ...
          {stages.map((stage) => (
            <article className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4" key={stage.id}>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-400 sm:grid-cols-4">
                <span>{stage.rosterCount} players</span>
                <span>{stage.batchCount} rooms</span>
                <span>{stage.promotedCount} promoted</span>
                <span>{stage.eliminatedCount} eliminated</span>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/50 p-4">
          ...
          <div className="mt-3 overflow-hidden rounded-xl border border-slate-800">
            {resultPage.items.map((item) => (
              <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] gap-3 border-t border-slate-800 px-3 py-2 text-sm first:border-t-0" key={`${item.room}:${item.rank}:${item.player?.profileTag}`}>
                <span className="min-w-0"><strong className="block truncate text-white">{item.player?.displayName || "Player"}</strong><small className="text-slate-500">{item.player?.profileTag}</small></span>
                <span className="text-slate-400">Room {item.room}</span>
                <span className="font-black text-cyan-200">#{item.rank}</span>
              </div>
            ))}
            {resultPage.status !== "loading" && !resultPage.items.length ? <p className="p-3 text-sm text-slate-500">No {resultView} players recorded yet.</p> : null}
          </div>
      ...
      {canConfigure && currentPlayers >= 2 ? (
        <form className="mt-5 rounded-2xl border border-slate-800 bg-slate-950/50 p-4" onSubmit={submit}>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <NumberField label="Players per room" max={100} min={2} onChange={(value) => setField("participantsPerMatch", value)} value={definition.participantsPerMatch} />
            {!definition.finalRound ? <NumberField label="Promote per room" max={99} min={1} onChange={(value) => setField("advanceCount", value)} value={definition.advanceCount} /> : <Fact label="Result" value="Final ranking" />}
          ...
          <button className="mt-4 rounded-xl bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950 disabled:opacity-50" disabled={actionBusy} type="submit">
